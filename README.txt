...
Currently I have separate scripts for the steps, but after I optimize the program I plan to combine them.
...
- Function: plotmotif(Boolean (True for overlay, False for subplot), cutoff_value, coefficient value (for clustering))

Plot motif plots the -log(pvalue) of the terms and the user has the option to either have the graphs as overlays (True) or as subplots (False) given a list of terms, a cutoff value, and a coefficient value. The p-values were corrected with multiple-hypothesis (MH) testing adjustment by multiplying by the # of motifs and # of terms. 
Overlay
